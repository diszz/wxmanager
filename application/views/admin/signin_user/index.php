                    ...
                    <div class="panel panel-default">
                    	...
                    	<div class="panel-body">
                    		...
                    		<div class="form-group">
	                    		<a href="/admin/signin_user/index/" class="btn <?php echo !$cur_signin ? 'btn-danger' : ''?>">全部</a>
	                    		<?php foreach ($signinList as $item){?>
	                    		<a href="/admin/signin_user/index/<?php echo $item['signin_id']?>" class="btn <?php echo $cur_signin == $item['signin_id'] ? 'btn-danger' : ''?>"><?php echo $item['name']?></a>
	                    		<?php }?>
                    		</div>
                    		</div>
                    		<div class="col-md-6">
                    		<div class="form-group">
	                    		<?php if (!$file){?>
								<a href="/admin/signin_user/export_code?code_id=<?php echo $cur_signin?>" class="btn btn-primary" >把筛选的结果数据导出成excel文件</a>
	                    		<?php }elseif($file_exist){?>
	                    		<a href="<?php echo str_replace(ROOTPATH, '/', $file)?>" class="btn btn-success" target="_blank">下载excel文件</a>
	                    		<?php }else{?>
	                    		<a href="/admin/signin_user/index/<?php echo $cur_signin?>?file=<?php echo base64_encode_($file)?>" class="btn btn-warning" >excel文件生成中, 点击刷新</a>
	                    		<?php }?>
                    		</div>
                    		</div>
                    	</div>
                    	...
                    	<div class="panel-body">
							...
							<thead>
								<tr class="success">
									...
									<th width="100">总次数</th>
									<th width="100">上次签收时间</th>
									<th width="100">连续次数</th>
							...
							<tbody>
								...
								<tr class="odd ">
									<td><?php echo $item['signin_user_id']?></td>
									<td><?php echo $item['signin_id']?></td>
									<td><?php echo $item['openid']?></td>
									<td><?php echo $item['total_count']?></td>
									<td><?php echo !empty($item['last_time']) ? date('Y-m-d H:i:s', $item['last_time']) : ''?></td>
									<td><?php echo $item['keep_count']?></td>
