                    ...
                    <div class="panel panel-default">
                    	...
                    	<div class="panel-body">
                    		...
                    		<div class="form-group">
	                    		<a href="/admin/lottery_record/index/" class="btn <?php echo !$cur_lottery ? 'btn-danger' : ''?>">全部</a>
	                    		<?php foreach ($lotteryList as $item){?>
	                    		<a href="/admin/lottery_record/index/<?php echo $item['lottery_id']?>" class="btn <?php echo $cur_lottery == $item['lottery_id'] ? 'btn-danger' : ''?>"><?php echo $item['name']?></a>
	                    		<?php }?>
                    		</div>
                    		</div>
                    		<div class="col-md-6">
                    		<div class="form-group">
	                    		<?php if (!$file){?>
								<a href="/admin/lottery_record/export_code?code_id=<?php echo $cur_lottery?>" class="btn btn-primary" >把筛选的结果数据导出成excel文件</a>
	                    		<?php }elseif($file_exist){?>
	                    		<a href="<?php echo str_replace(ROOTPATH, '/', $file)?>" class="btn btn-success" target="_blank">下载excel文件</a>
	                    		<?php }else{?>
	                    		<a href="/admin/lottery_record/index/<?php echo $cur_lottery?>?file=<?php echo base64_encode_($file)?>" class="btn btn-warning" >excel文件生成中, 点击刷新</a>
	                    		<?php }?>
                    		</div>
                    		</div>
                    	</div>
                    	...
                    	<div class="panel-body">
							...
							<tbody>
								...
								<tr class="odd ">
									<td><?php echo $item['lottery_record_id']?></td>
